  ...
  return `Compare this resume against the job description. Return ONLY valid JSON:
{
  ...
  "summaryRewrite": "tailored 3-sentence professional summary rewritten for this role",
      ...
      <div className="page-wrap">
        ...
        <div className="anim-up" style={{ marginBottom: 'clamp(18px,2.5vw,30px)' }}>
          ...
          <p style={{ fontSize: 'clamp(11px,1.1vw,13px)', color: 'var(--dim)', lineHeight: 1.7, maxWidth: 560 }}>
            Upload your resume, paste a job description — get a fit score, gap analysis, and tailored suggestions.
          </p>
        </div>

        {!result ? (
          <div className="anim-up d1" style={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
            {/* Resume upload + JD paste side by side */}
              ...
              <div className="cell" style={{ borderLeft: 'none' }}>
                ...
                <textarea
                  ...
                  style={{
                    ...
                    fontSize: 11,
                    lineHeight: 1.7,
                    borderLeft: '3px solid rgba(94,240,200,.25)',
                  }}
                />
              </div>
            </div>
          ...
          <MatchResult result={result} verdictCol={verdictCol} onReset={() => { setResult(null); setLR(''); setResumeText('') }} />
        )}
      </div>
    </Section>
  )
...
function MatchResult({ result, verdictCol, onReset }) {
  return (
    <div className="anim-up">
      {/* Verdict banner */}
      <div style={{
        padding: 'clamp(14px,2vw,22px) clamp(16px,2vw,26px)',
        background: 'var(--bg1)',
