      ...
      <div className="page-wrap">
        <div className="anim-up" style={{ marginBottom:'clamp(18px,2.5vw,30px)' }}>
          ...
          <p style={{ fontSize:'clamp(11px,1.1vw,13px)', color:'var(--dim)', lineHeight:1.7, maxWidth:560 }}>
            Upload your resume, select a section, and AI rewrites it to be stronger and ATS-optimized.
          </p>
        </div>
        ...
        {!result ? (
          <div className="anim-up d1" style={{ display:'flex', flexDirection:'column', gap:1 }}>
            {/* Upload zone */}
            <ResumeUploader onParsed={handleParsed} value={localResume} label="YOUR RESUME — PDF OR DOCX" accentColor="rgba(255,179,71,.3)" />

            {/* Config — only show after upload */}
            {localResume && (
              <>
                ...
                <button className="btn-lime" onClick={run}
          ...
          <FixResult result={result} copied={copied} onCopy={copy} onReset={() => setResult(null)} />
        )}
      </div>
    </Section>
  )
